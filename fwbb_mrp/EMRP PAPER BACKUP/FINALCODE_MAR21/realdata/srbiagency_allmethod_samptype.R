...
#-- uses SRBI+Agency
#-- accts for sample type in outcome model
...
# need to relabel XxZ combinations s.t. we incorporate samptype
samp <- samp %>% dplyr::select(-J_cell)
n <- nrow(samp)
grptbl <- samp %>% group_by(age3,sex2,race3,educat4,povgap4,svefreq2, aecat,samptype) %>% summarise(
...
xj <- samp$J_cell
grptbl <- samp %>%
  group_by(age3,sex2,race3,educat4,povgap4,svefreq2, aecat,x1_popcts,samptype) %>%
...
sampgroup <- as.numeric(grptbl$samptype)-1
zpopcts <- grptbl$zpopcts

stanfitNq <- stan(file = "mrp2_Nq_samptype.stan",
                  data = c("Ma", "Mb", "Mc", "Md", "Me", "L", "Mae", "J_stan", "n", "y", "xj",
                           "agroup", "bgroup", "cgroup", "dgroup", "egroup", "aegroup","xgroup", "sampgroup"),
...
grptbl <- samp %>%
  group_by(age3,sex2,race3,educat4,povgap4,svefreq2, aecat,samptype) %>%
  summarise(n())
...
stanfit <- stan(file = "outcome_model_samptype.stan",
...
write.csv(resmat_wmrp, "srbiagency_samptype_wfpbbmrp.csv")
write.csv(resmat_mmrp, "srbiagency_samptype_mmrp.csv")
write.csv(resmat_mrp2, "srbiagency_samptype_mrp2.csv")
write.csv(njmat_wmrp, "srbiagency_samptype_wfpbbmrp_nj.csv")
write.csv(njmat_mmrp, "srbiagency_samptype_mmrp_nj.csv")
write.csv(njmat_mrp2, "srbiagency_samptype_mrp2_nj.csv")
