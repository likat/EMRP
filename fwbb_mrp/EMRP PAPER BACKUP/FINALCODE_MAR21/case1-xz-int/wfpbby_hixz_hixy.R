...
bbcellmean <- matrix(nrow=F_draw,ncol=J)
tempcellmean <- matrix(nrow=L,ncol=J)
cellmean <- matrix(nrow=sim,ncol=J)
## Nj summary
...
for(i in 1:sim){
  ...
  for(l in 1:L){
    ...
    boostj <- unique(sampled$J_cell[unique_ind])
    fpbb_temp <- rep(0, N)
    bbcellmean <- matrix(nrow=F_draw,ncol=J)
    for(f in 1:F_draw){
      fpbb_temp <- wtpolyap(ysamp = unique_ind, wts = twt, k = N-length(unique_ind))
      synthpop <- sampled[fpbb_temp,c("J_cell","subgroup", "Y", "subgroup6", "subgroup7")]
      jmean <- synthpop %>% group_by(J_cell) %>% summarise(cellmean=mean(Y))
      bbcellmean[f,boostj] <- jmean$cellmean
    ...
    tempcellmean[l,] <-colMeans(bbcellmean, na.rm=T) 
  ...
  # cellpicked[sampled_J] <- cellpicked[sampled_J] + 1
  #- bias, SE, rMSE
  cellmean[i,] <- colMeans(tempcellmean, na.rm=T)
  cellmeanbias[sampled_J] <- cellmeanbias[sampled_J]+colMeans(cellmean[,sampled_J],na.rm=T)-popmeans[sampled_J]
  cellmeanrmse[sampled_J] <- cellmeanrmse[sampled_J] + cellmeanbias[sampled_J]^2
  
  #- coverage
  CI_cellmean <- apply(cellmean[,sampled_J], 2, quantile, c(0.025, 0.975), na.rm=T)
...
write.csv(colMeans(CIlength_cellmean,na.rm=T), "cellmeanCI_wfpbby.csv")
write.csv(colMeans(CI_cellmeancov,na.rm=T), "cellmeancov_wfpbby.csv")