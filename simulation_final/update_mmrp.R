...
update.mmrp <- function(){
  ...
  for(s in 1:sim){
    ...
    for(m in sampled_z){
        wts_mrp[m,] <- rowMeans(rmultinom(zpopcts_lengthZ[m], 1, prob = zrelfreq[m,]))*zpopcts_lengthZ[m]
    }
  ...
  y_bar_mmrp5[i] <<- mean(mrp_posterior5,na.rm=T)
  ...
  CI_mmrp5[i,] <<- quantile(mrp_posterior5, c(0.025, 0.975),na.rm=T)
  
  if(is.na(between(true_mean, CI_mmrp[i,1], CI_mmrp[i,2]))){break}
  if(between(true_mean, CI_mmrp[i,1], CI_mmrp[i,2])){CI_covered_mmrp[i] <<- 1}else if(sum(is.na(CI_mmrp[i,])!=0)){CI_covered_mmrp[i] <<- NA}else{CI_covered_mmrp[i] <<- 0}
  ...
  if(between(true_mean5, CI_mmrp5[i,1], CI_mmrp5[i,2])){CI_covered_mmrp5[i] <<- 1} else{CI_covered_mmrp5[i] <<- 0}
