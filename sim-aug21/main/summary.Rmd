---
title: "Summary of 0707 binarypinc main"
output: pdf_document
---
...
$$
logit(P(X_i=1|Z)) = \Gamma_0 + \gamma_{Za[i]} + \gamma_{Zb[i]} + \gamma_{Zc[i]} + \gamma_{Za[i],Zc[i]}+\gamma_{Zb[i],Zc[i]}
$$
...
\gamma_{Zc[i]}&=(0,-1)\\
\gamma_{Za[i],Zc[i]} &= (0,0,0,0,0)\\
\gamma_{Zb[i],Zc[i]} &= (0,0,0,0,0)
\end{align*}
...
#- coverage
  ...
  ggplot(data, aes(Method, Inference, fill= Z)) + 
    geom_tile()+ labs(title = "95% Average confidence interval length for MAIN case", fill = "length")+scale_fill_continuous(high = "#28629c", low = "#56B1F7")
...
## == Cellmean SE ==
  ...
  ggplot(data, aes(x=X,y=Y, fill= Z)) + 
    ...
    theme(axis.title.x = element_blank()) +
    theme(axis.title.y = element_blank()) +
    labs(title = "Standard error of estimated population cell count, MAIN case", fill = "SE")+
    scale_fill_continuous(high = "#28629c", low = "#56B1F7")+
    facet_grid(. ~ Method)
