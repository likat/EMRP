...
bayesvar <- bayesvar1 <- bayesvar2 <- bayesvar3 <- bayesvar4 <- bayesvar5 <- rep(NA, iter)
## general summary
bias <- rMSE <- SE <- 0
...
set.seed(stanfitrand[i])  
 ...
 cellmeans_stan <- extract(get(modelname[i]),permuted = TRUE, inc_warmup = FALSE,include = TRUE)$cellmean
  ...
  bayesvar[i] <- var(mrp_posterior,na.rm=T)
  bayesvar1[i] <- var(mrp_posterior1,na.rm=T)
  bayesvar2[i] <- var(mrp_posterior2,na.rm=T)
  bayesvar3[i] <- var(mrp_posterior3,na.rm=T)
  bayesvar4[i] <- var(mrp_posterior4,na.rm=T)
  bayesvar5[i] <- var(mrp_posterior5,na.rm=T)
...
resmatfun <- function(ybarmat, truemean, CI, CIcovered,labs,bayesvarmat){
  ...
  res <-rbind(rMSE, bias, SE, bayesse, CI_length, coverage_rate)
  colnames(res) <- labs
  return(res)
}
